    ...
    while cap.isOpened():
        ...
        if not game_started:
            ...
            if game_over:
                ...
                # 새로운 두더지 추가
                if not mole_list or current_time - mole_list[-1][2] >= mole_duration:
                    mole_x = random.randint(0, frame.shape[1] - mole_width)
                    mole_y = random.randint(0, frame.shape[0] - mole_height)
                    mole_list.append((mole_x, mole_y, current_time))

                # 두더지 그리기 및 사라진 두더지 임시 리스트에 저장
                removed_moles = []
                for mole_x, mole_y, mole_start_time in mole_list:
                    if current_time - mole_start_time >= mole_duration:
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks in results.multi_hand_landmarks:
                        hand_x = int(hand_landmarks.landmark[8].x * frame.shape[1])
                        hand_y = int(hand_landmarks.landmark[8].y * frame.shape[0])
